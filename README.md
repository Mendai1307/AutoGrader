...
AutoGrader 并非「在网页中嵌入一次 AI 调用」的常规实现。它将 AI 推理置于
**LearnBuddy 对话侧**（专家智能体在该环境中运行），产出结构化的 `ReviewResult` JSON 并提交进仓库；
**浏览器端运行时零 AI 调用、零后端**（工程依赖中不存在任何第三方大模型 SDK），只承担确定性渲染与纯前端规则复核。

该架构范式称为 **Compile-time AI**，由此获得的三项保证正是本作品的核心论证点：

| 核心保证 | 成立依据 |
|---|---|
| **结果可复现** | 同一份 `ReviewResult` 在任何机器上渲染出完全相同的页面；分数由纯算术得出，无模型参与 |
| **过程可审计** | 每个分数都可追溯到评分点 → 证据引用 → 报告原文的逐字位置；摘要与原文由工具逐字比对，**不做任何模糊归一化** |
...
| # | 交付物 | 角色 | 状态 |
|---|---|---|---|
| 一 | **专家智能体包**（Prompt + Skills + Tools + Workflow + Evaluation） | 能力本体，AI 在这里运行 | ✅ **五件套 + 打包层全部落地**，已通过官方校验，可直接交给 LearnBuddy 本地召唤 |
| 二 | **Web Demo 产品展示页** | 产品形态（在线链接 + 源码仓库） | ✅ **7 个路由全部落地并实测**；线上链接：<https://mendai1307.github.io/AutoGrader/> |
| 三 | **评审材料包**（PPT / 3 分钟视频 / 对话记录归档） | 佐证链 | ✅ **对话记录归档（`documents/dev_log/`）+ 作品介绍 PPT（18 页）+ 演示视频台本（v1.0）已交付** |

上一版（v0.1）已交付的基础：5 页面静态站、12 份合成样例 + 教师金标准 + 12 份真实评阅结果（MAE 3.05）。
本项目以 v0.1 资产为**复用基线**，并将其移植为 1.2.0 契约下的合规数据（见 §五）。

## 三、运行与验证

工具链仅依赖 **Python 标准库**，约定 `python3` 优先、`python` 回退。
...
`npm run build` 已串联**三道构建期门禁**——任一道未通过即不产出构建结果：

| 门禁 | 检查内容 |
|---|---|
| `sync-assets` | 将随包资产同步进 `web/`，使站点**自包含**（不依赖后端） |
| `contract-check` | Zod 契约与 JSON Schema 的**对象层 / 字段差异**（实测 23 个对象层 / 132 字段零差异） |
| `check-fonts` | 中文字体子集的**码位覆盖**（防止真机出现缺字） |

另有独立的跨端对拍与产物自检命令：

```bash
npm run audit:cross-end    # 浏览器端规则引擎 ↔ 工具链 T2，同一输入必须同结论（实测 12/12）
npm run preview:check      # 启动静态服务并做冒烟检查
...
# 指纹算法跨语言一致性（Python 与 JS 两侧必须逐字相同）
...
**退出码（全局约定，CI 依赖该约定）**：`0` 正常 ｜ `1` 可继续但需注意 ｜ `2` 输入不可读或非法 ｜
**`3` 降级运行（校验未跑全 → CI 视为不通过）**。
...
## 四、Web Demo 路由概览（7 个路由）

| 路由 | 页面 | 功能要点 |
|---|---|---|
| `/` | 首页 | 以三句话阐明架构范式：推理在对话侧完成 / Web 只做确定性渲染 / 每个结论可回溯 |
...
| `/eval` | 一致性评测 | 总分 MAE / 逐项命中率 / 档位一致率 / 置信度校准 ECE，**浏览器端实时计算**（每个数字都可追溯到一行可查看的 JS）+ 4 张零依赖 SVG 图 |
| `/trace` | 工作流溯源（导航标签为「溯源与指纹」） | 五 Agent 编排逐环节展示，指纹复算结论与 `/report/[id]` **同源** |

## 五、核心口径速查（已冻结，不得改写）

- **总分** = `Σ (score / maxScore) × weight` —— **不是** `Σ (score × weight / 100)`
- **权重序列**（和 = 100）：`5 / 12 / 5 / 7 / 20 / 10 / 11 / 13 / 7 / 5 / 3 / 2`
- **待复核项不计入求和、不做归一化** → 总分为**部分分（下界）**，必须携带 `W_r`
  （归一化会使「教师认真复核」表现为分数下降）
...
| `对话记录索引.md` | **53 轮逐轮索引**，五要素齐备（日期 / 目的 / 结论 / 产物 / 对话 ID）+ 指向逐字转录的可点击链接 |
...
凡属「机器执行了什么」的，压缩为一行（工具名 + 关键参数 + 成功与否）；工具返回正文、内部推理、
系统注入内容已移除。**含 2 轮被中断 / 无结论的会话（C-000、C-027），未做美化删除。**

导出脚本带**完整性断言**：磁盘上每个会话记录文件都必须被登记，否则中止导出——
**不存在「只挑选效果较好的几轮放入归档」的可能**。

## 七、当前进度

| 交付物 | 状态 |
|---|---|
| **一 · 专家智能体包** | ✅ **全部落地**。五件套齐备；契约冻结 **1.2.0**；六脚本自测 **61 项**全通过；打包层装配为平台合规专家包，**已通过官方 `validate_expert.py` 校验并注册到 my-experts 市场**（专家中心可召唤）。**头像已提交**（`avatars/expert.png`，512×512 PNG），官方校验器**告警归零** |
| **二 · Web Demo** | ✅ **7 个路由全部落地并实测**：`tsc` 0 错、三道门禁全绿、跨端对拍 12/12、渲染核验 **39 条断言** + 1 个「字面 markdown」扫描段（2026-09-26 文案规范化改写后修订：删 3 条 `/trace` 指纹复算断言以对齐队友提交 `1d0f208` 的页面精简——该展示现仅保留于 `/report/[id]`；「字面 `**`」扫描由 `/agents` 单路由扩至 5 个路由，合计 0 处；全量断言已在构建期预渲染产物上逐条核验通过）、`next build` 静态页 21/21。线上链接：<https://mendai1307.github.io/AutoGrader/> |
...
| # | 项 | 说明 |
|---|---|---|
| 1 | **本机无法产出 `out/`** | 宿主 `safe-delete` 拦截所致（**环境问题，非代码问题**）。构建本身成功：`next build` 的编译 ✓ / 类型检查 ✓ / 静态页生成 21/21 ✓ |
| 2 | 两种编排形态的实跑对照 | `workflow/W2` 给出**可校验的等价性判据**；真正的「两种形态评阅同一份报告」需**真实 LearnBuddy 会话** |
| 3 | **3 分钟 Demo 视频** | 台本 v1.0 已产出；**成片待剪辑**——录屏素材、AI 生成画面、BGM 均未产出（线上链接已发布，「录屏会暴露 `localhost`」这一原有阻塞已解除） |
| 4 | `LICENSE` | 未添加 |
...
│   ├── video/                      #   3 分钟演示视频：台本 v1.0 已产出，成片待剪辑
...
│   ├── dev_log/                    #   对话记录归档（§3.3）：README + 索引 + 逐字转录
...
│   ├── app/                        #   7 个路由
│   ├── components/                 #   可复用组件（note / charts / eval-metrics / deep-link-button …）
