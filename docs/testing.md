...
| 10 | 前端类型检查（本轮新增） | `npx tsc --noEmit`（在 `frontend/` 下，Node v22.22.2） | **通过，0 error** |
...
> 本轮复算（仅 12 份真实结果、共 144 项）：`itemFailures = 0`、`filesWithBadItems = 0`，
> 即 144 / 144 通过。该结论现**直接显示在页面上**：`/report/[id]` 分数面板的「单项档位自证」显示 `12 / 12`。
...
1. **没有自动化测试框架**：`frontend/package.json` 的 scripts 只有 `dev` / `build` / `start` /
   `preview` / `preview:check` / `lint`，未引入 jest / vitest / playwright 等；无测试目录、无单测用例。
   本项目所有验证都是**一次性、确定性、可复现的复算**，不是回归测试套件。
   > 更新：仓库现有 CI 配置 `.github/workflows/deploy.yml`，但它的职责是**构建并发布**静态产物
   > （`npm ci` → `npm run build` → 发布 `frontend/out`），**不跑任何测试**；
   > 因此"无回归测试"这一条仍然成立。
...
3. **未实现"浏览器端一键复算指纹"**：`contract.md` 第五节的页面侧建议中列举了此功能，
   ...
   > 补充：`/report/[id]` 分数面板新增的「**分数自证**」是**构建期**用
   > `verifyItemScore()` / `verifyTotalScore()` 做的契约复算（核验"得分 = 满分 × 档位系数"与加权总分口径），
   > 结果固化进 HTML。它证明"分数由档位机械推出"，但**不等于**浏览器端 Web Crypto 复算，不要混淆。
4. **核查器已接入页面**（本条已更新）：`lib/inspectors/` 经 `lib/data.ts` 调用，
   渲染在 `/report/[id]` 的「客观核查」区块与查重指纹区。第四节的结论现在**直接可见于页面**，
   不再需要"独立运行该模块"才能看到。**仍未做**：全量 66 对的相似度矩阵 / 榜单页面。
...
7. **`backend/` 无可执行验证**：其中只有 prompt 规格文档（`.md`），不存在可运行的评阅实现
   （早前的 5 个 0 字节 `.py` 占位文件**已在后续提交中删除**，`backend/agents/` 现无任何可执行文件），
   也因此**不存在"跑一遍后端看结果"的验证路径** —— 这是架构前提，不是缺陷。
8. **本轮改动的类型检查**：本轮对 `app/report/[id]/page.tsx` 做了前端改动（接入 `verifyItemScore`），
   已执行 `npx tsc --noEmit` 并**通过（0 error）**；按约定**未执行** `npm run build`，
   故 `frontend/out/` 仍是改动前的产物，需由另一位工程师统一重建。

### 5.1 曾发现的"文档与实现不一致"（本轮已全部修正）

以下几处是早前整理时**只读核查**发现的不一致。本轮已连同其他滞后表述一并修正，
保留在此仅为记录问题曾经存在，**下列描述已不反映当前仓库状态**：

1. ~~`frontend/app/eval/page.tsx` 的文件头注释已过期~~ —— **已核实为已修复**。
   该文件头现写作"public/results/ 下已有 12 份真实评阅结果…… 总分 MAE 3.05（N = 12）"，
   与实际一致，无需再改。
2. ~~`README.md` 第四节的合规声明仍称 `backend/agents/` 下是五个 0 字节 `.py` 占位文件~~ ——
   **已修正**：README 现如实写明该目录只有 5 份 `.md` 规格，且 5 个 `.py` 占位文件**已被删除**。
3. ~~README 称 `results/` 为"运行时拉取"~~ —— **已修正**：现写明为**构建期用 `node:fs`
   读取并渲染进 HTML**，`out/results/` 只是 `public/` 的静态拷贝。

4. **统计口径差异（非错误，但值得知道）**：以 `sample-01` 为例，同一份报告不同环节给出的数字并不等价 ——
...
> 历史记录（已不再适用）：早前整理曾尝试删除 `backend/agents/` 下五个 0 字节 `.py` 文件，
> 当时被本机删除守卫拦截（`SAFE_DELETE_BULK_REJECTED`）且未获授权，文件按原样保留。
> **该 5 个 `.py` 文件已在后续提交中被删除**，`backend/agents/` 现只有 5 份 `.md` 规格文档，
> 目录内不含任何可执行代码。
...
- **不可信/未证的部分**：Web 端不重跑推理；浏览器端不做 Web Crypto 指纹复算（页面上的"分数自证"是构建期复算）；
  无回归测试框架（CI 只构建部署、不跑测试）；低分档偏差偏大；样本量小、金标准单人标注。
