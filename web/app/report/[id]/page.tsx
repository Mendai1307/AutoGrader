...
export default async function ReportDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  /**
   * 契约级复算（2.4）：把 `total` 块的口径摊开给读者。
   * 关键点 —— **上界是 `Σ已计入 weight`（= weightIncluded），不是 `100 − weightExcluded`**；
   * 且待复核项不计入求和、不做归一化，所以总分是**部分分（下界）**。
   * 这两条不写出来，读者会把「部分分」误读成「扣过分」。
   */
  const audit = getContractAudit(report.id);

  /**
   * 指纹现场复算（2.4）：用 lib/fingerprint.ts 对结果文件重算 SHA-256 并与声明值比对。
   * 这一步让「结果有没有被改动过」不靠信任、靠复算 —— 它是"可审计"这句话的凭据。
   * 走 Web Crypto 故为 async；在静态导出下即构建期算好并固化。
   */
  const fingerprintCheck = await getFingerprintCheck(report.id);
      ...
      <main className="container pb-4">
        ...
        <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,24rem)]">
          <Card className="p-6">
            ...
            {/* 2.6 深链唤起：复核要在 LearnBuddy 对话侧发生，这里负责把用户送过去。
                有结果时动作是「复核」（模式 3），无结果时是「评阅」（模式 2）。 */}
            <div className="mt-5 border-t border-border pt-4">
              <DeepLinkButton
                reportId={report.id}
                title={report.title}
                action={state.status === 'ok' ? 'recheck' : 'review'}
              />
            </div>
          ...
          <Card className="p-6">
            {state.status === 'ok' && summary !== null ? (
              <div className="space-y-4">
                ...
                {/* ================= total 块口径（2.4）=================
                    不写这一段，读者会把「部分分」误读成「扣过分」：
                    待复核项不计入求和、不做归一化，故总分是下界，须连未计入权重一起看。 */}
                {audit === null ? null : (
                    ...
                    <dl className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1.5 text-[11px]">
                      ...
                      {audit.declaredTotal?.grade == null ? null : (
                        <div className="col-span-2 flex items-baseline justify-between gap-2">
                          <dt className="text-muted-foreground">结果声明的等级</dt>
                          <dd className="font-semibold">{audit.declaredTotal.grade}</dd>
                        </div>
                      )}
                    </dl>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="rounded-md bg-secondary/50 px-3 py-2">
                    ...
                    <div
                      className={cn(
                        ...
                        // 偏差程度用深浅阶表达（ok 深 / notice 弱 / bad 赭石），不用第二色系
                        TONE[deltaTone(Math.abs(summary.delta), 2, 5)].text,
                      )}
                    >
                  ...
                  <div className="rounded-md bg-secondary/50 px-3 py-2">
                    ...
                    <div className="mt-0.5 flex items-center gap-1 text-sm font-semibold tabular-nums">
                      {summary.check.ok ? (
                        <>
                          <CircleCheckBig className={cn('h-3.5 w-3.5', TONE.ok.icon)} />
                          <span className={TONE.ok.text}>{formatScore(summary.check.expected)}</span>
                        </>
                      ) : (
                        <>
                          <TriangleAlert className={cn('h-3.5 w-3.5', TONE.bad.icon)} />
                          <span className={TONE.bad.text}>不一致</span>
                        </>
                      )}
                    </div>
                  </div>
                  <div className="rounded-md bg-secondary/50 px-3 py-2">
                    ...
                    <div className="mt-0.5 flex items-center gap-1 text-sm font-semibold tabular-nums">
                      {summary.itemFailures.length === 0 ? (
                        ...
                        <>
                          <TriangleAlert className={cn('h-3.5 w-3.5', TONE.bad.icon)} />
                          <span className={TONE.bad.text}>{summary.itemFailures.length} 项不一致</span>
                        </>
                      )}
                    </div>
                  </div>
                ...
                {summary.check.ok ? null : (
                  <p
                    className={cn(
                      'flex items-start gap-2 rounded-md border p-3 text-[12px] leading-relaxed text-foreground',
                      TONE.bad.card,
                    )}
                  >
                    <CircleAlert className={cn('mt-0.5 h-3.5 w-3.5 shrink-0', TONE.bad.icon)} />
              ...
              <div className="space-y-4">
                ...
                <dl className="space-y-1.5 text-xs">
                  ...
                  <div className="flex justify-between gap-3">
                    ...
                    <dd className={cn('font-medium', state.status === 'invalid' && TONE.bad.text)}>{status.label}</dd>
                  </div>
                </dl>
        ...
        {state.status === 'invalid' ? (
          <Card className={cn('mt-4 p-5', TONE.bad.card)}>
            <div className="flex items-start gap-3">
              <TriangleAlert className={cn('mt-0.5 h-4 w-4 shrink-0', TONE.bad.icon)} />
              ...
              <>
                ...
                <Card className="p-5">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Badge variant={state.data.review.triggered ? 'warning' : 'accent'}>
                ...
                <Card className="p-5">
                  ...
                  <div className="mt-4 space-y-3">
                    <div>
                      <div className={cn('text-xs font-medium', TONE.ok.text)}>亮点</div>
                      <ul className="mt-1.5 space-y-1.5">
                        {state.data.feedback.strengths.map((text, index) => (
                          <li key={`strength-${index}`} className="flex gap-2 text-[13px] leading-relaxed">
                            <CircleCheckBig className={cn('mt-0.5 h-3.5 w-3.5 shrink-0', TONE.ok.icon)} />
                            <span>{text}</span>
                          </li>
                        ))}
                      </ul>
                    ...
                    <div>
                      <div className={cn('text-xs font-medium', TONE.notice.text)}>改进建议</div>
                      <ul className="mt-1.5 space-y-1.5">
                        {state.data.feedback.improvements.map((text, index) => (
                          <li key={`improvement-${index}`} className="flex gap-2 text-[13px] leading-relaxed">
                            <TriangleAlert className={cn('mt-0.5 h-3.5 w-3.5 shrink-0', TONE.notice.icon)} />
                            <span>{text}</span>
                          </li>
                        ))}
                      </ul>
        ...
        <section className="mt-8">
          <ProvenanceBlock
            ...
            hint={
              state.status === 'ok'
                ? '以下信息随结果文件一同固化；指纹一栏是本次现场复算的结果 —— 不一致即说明文件被改动过。'
