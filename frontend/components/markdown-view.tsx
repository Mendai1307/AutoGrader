    ...
    <div className={cn('text-sm leading-relaxed text-foreground/90', className)}>
      {blocks.map((block, index) => {
        ...
        switch (block.kind) {
          ...
          case 'code':
            return (
              <figure key={key} className="my-4 overflow-hidden rounded-lg border border-border bg-slate-950">
                <figcaption className="flex items-center justify-between border-b border-white/10 px-3 py-1.5 text-[11px] text-slate-400">
                    ...
                    <code className="block font-mono text-slate-100">
                      {block.lines.map((codeLine, lineIndex) => (
                        <span key={`${key}-l${lineIndex}`} className="flex">
                          <span className="w-11 shrink-0 select-none pr-3 text-right text-slate-600 tabular-nums">
