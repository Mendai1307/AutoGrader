 ...
 * ---------------------------------------------------------------------------
 * 把 frontend/lib/inspectors/ 的确定性核查结果渲染成可核验的事实清单：
 *   ① 章节完整性   ② 代码块与语言分布 + 关键 API 命中
 *   ③ 统计量（字数 / 行数 / 图片数）   ④ 图表引用核查
 *   ⑤ findings 结论列表（error / warn / info 分级着色）
...
const LEVEL_META: Readonly<Record<InspectionLevel, { label: string; chip: string; icon: typeof Info }>> = {
  error: { label: '错误', chip: 'border-rose-200 bg-rose-50 text-rose-700', icon: CircleAlert },
  warn: { label: '注意', chip: 'border-amber-200 bg-amber-50 text-amber-700', icon: TriangleAlert },
  info: { label: '通过', chip: 'border-emerald-200 bg-emerald-50 text-emerald-700', icon: CircleCheckBig },
};

const GRADE_META: Readonly<Record<SimilarityGrade, { label: string; chip: string; bar: string }>> = {
  duplicate: { label: '高度疑似重复', chip: 'border-rose-200 bg-rose-50 text-rose-700', bar: 'bg-rose-500' },
  suspicious: { label: '需人工复核', chip: 'border-amber-200 bg-amber-50 text-amber-700', bar: 'bg-amber-500' },
  distinct: { label: '正常区分', chip: 'border-emerald-200 bg-emerald-50 text-emerald-700', bar: 'bg-emerald-500' },
};
      ...
      <CardHeader className="border-b border-border bg-secondary/40">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="ml-auto flex flex-wrap items-center gap-1.5">
            <Badge className={LEVEL_META.error.chip}>错误 {levelCounts.error}</Badge>
            <Badge className={LEVEL_META.warn.chip}>注意 {levelCounts.warn}</Badge>
            <Badge className={LEVEL_META.info.chip}>通过 {levelCounts.info}</Badge>
          </span>
        </div>
        <p className="text-xs leading-relaxed text-muted-foreground">
          以下数据由 <code className="font-mono">frontend/lib/inspectors/</code> 的纯函数核查器在
      ...
      <CardContent className="space-y-5 pt-5">
        ...
        <div className="space-y-2.5 border-t border-border pt-4">
          ...
          <Progress
            ...
            barClassName={chapters.missingCount === 0 ? 'bg-emerald-500' : 'bg-amber-500'}
            ...
            {chapters.items.map((item) => (
              <li
                ...
                className={cn(
                  ...
                  item.present ? 'border-border' : 'border-amber-200 bg-amber-50/60',
                )}
              >
                {item.present ? (
                  <CircleCheckBig className="h-3.5 w-3.5 shrink-0 translate-y-0.5 text-emerald-600" />
                ) : (
                  <TriangleAlert className="h-3.5 w-3.5 shrink-0 translate-y-0.5 text-amber-600" />
                )}
        ...
        <div className="space-y-2.5 border-t border-border pt-4">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {codes.untaggedBlockCount === 0 ? null : (
              <Badge className={LEVEL_META.warn.chip}>{codes.untaggedBlockCount} 块未标注语言</Badge>
            )}
            {codes.unclosedBlockCount === 0 ? null : (
              <Badge className={LEVEL_META.warn.chip}>{codes.unclosedBlockCount} 块围栏未闭合</Badge>
            )}
          </div>

          {codes.hits.length === 0 ? (
            <p className="flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50/60 p-2.5 text-[12px] leading-relaxed text-amber-900">
        ...
        <div className="space-y-2.5 border-t border-border pt-4">
          ...
          >
            ...
            {figures.unnumberedCount === 0 ? null : (
              <Badge className={LEVEL_META.warn.chip}>{figures.unnumberedCount} 张缺图号</Badge>
            )}
            {figures.uncaptionedCount === 0 ? null : (
              <Badge className={LEVEL_META.warn.chip}>{figures.uncaptionedCount} 张缺题注</Badge>
            )}
          </SectionTitle>
            ...
            <ul className="space-y-1.5">
              {unreferenced.map((figure) => (
                <li
                  ...
                  className="flex flex-wrap items-baseline gap-2 rounded-md border border-amber-200 bg-amber-50/60 px-2.5 py-1.5 text-[12px]"
                >
        ...
        <div className="space-y-2.5 border-t border-border pt-4">
          ...
          <p className="flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50/60 p-2.5 text-[11px] leading-relaxed text-amber-900">
            <TriangleAlert className="mt-0.5 h-3.5 w-3.5 shrink-0" />
            <span>
