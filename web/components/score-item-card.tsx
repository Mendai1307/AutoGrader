...
function EvidenceItem({ evidence }: { evidence: Evidence }) {
  ...
  const confidenceMeta = CONFIDENCE_GRADE_META[grade];
  return (
    <li className="rounded-md border border-border bg-secondary/30 p-3">
      <div className="flex flex-wrap items-center gap-2 text-[11px] text-muted-foreground">
        ...
        {/* citationValid 是 T3 逐字比对的结论；缺省表示"没做逐字校验"，与 false 不同 */}
        {evidence.citationValid === undefined ? null : evidence.citationValid ? (
          <Badge variant="accent">引用逐字相符</Badge>
        ) : (
          <Badge variant="danger">引用与原文不符</Badge>
        )}
        <Badge className={cn('ml-auto', confidenceMeta.chip)}>
          <span aria-hidden>{confidenceMeta.symbol}</span>
          证据置信度 {formatRatio(evidence.confidence)}
        </Badge>
      </div>
      ...
      {/* 溯源链路：锚回 T1 的结构块，回答"这句话是从哪个块里摘出来的" */}
      {evidence.blockRef === undefined ? null : (
        <p className="mt-2 flex flex-wrap items-center gap-1.5 font-mono text-[11px] text-muted-foreground">
          <span className="not-italic">结构块</span>
          <span className="rounded bg-background px-1 py-0.5">{evidence.blockRef.blockId}</span>
          <span>@ {evidence.blockRef.anchor}</span>
          <span className="break-all">digest {evidence.blockRef.digest.slice(0, 16)}…</span>
        </p>
      )}
...
export function ScoreItemCard({ item, rubricItem, adjustment }: ScoreItemCardProps) {
  ...
  const confidenceMeta = CONFIDENCE_GRADE_META[grade];
  const scored = item.score !== null && item.score >= item.maxScore;
  /** 该项是否计入总分求和（pending 或无终值项不计入） */
  const counted = isCountedInTotal(item);
    ...
    <Card className="overflow-hidden">
      <div className="flex flex-wrap items-start gap-2 border-b border-border bg-secondary/30 px-4 py-3">
        ...
        <Badge className={level.chip}>
          <span aria-hidden className="font-mono tracking-tighter">{level.symbol}</span>
          {level.label}
        </Badge>
        {/* 两个标记语义正交，必须分别显示：needsReview 不影响总分，pending 不计入求和 */}
        {item.needsReview ? (
          <Badge variant="warning" title={NEEDS_REVIEW_META.description}>
            <span aria-hidden>{NEEDS_REVIEW_META.symbol}</span>
            {NEEDS_REVIEW_META.label}
          </Badge>
        ) : null}
        {item.pending === true ? (
          <Badge className={PENDING_META.chip} title={PENDING_META.description}>
            <span aria-hidden>{PENDING_META.symbol}</span>
            {PENDING_META.label}
          </Badge>
        ) : null}
      </div>
      ...
      <div className="space-y-3 px-4 py-4">
        <div className="flex flex-wrap items-end gap-x-5 gap-y-2">
          ...
          <div>
            <div className="text-xs text-muted-foreground">{counted ? '本项贡献' : '本项贡献（未计入）'}</div>
            <div className={cn('text-sm font-medium tabular-nums', !counted && 'text-muted-foreground line-through')}>
              {item.score === null
                ? '—'
          ...
          <div className="ml-auto text-right">
            ...
            <Badge className={confidenceMeta.chip}>
              <span aria-hidden className="font-mono tracking-tighter">{confidenceMeta.symbol}</span>
              {formatRatio(item.confidence)} · {formatConfidence(item.confidence)}
            </Badge>
          </div>
        </div>

        {/* 教师改过分：契约不变式 I7 要求能看出"改之前是多少" */}
        {item.overriddenByTeacher === true ? (
          <p className="flex flex-wrap items-center gap-2 rounded-md border border-primary/40 bg-primary/5 px-3 py-2 text-[12px] leading-relaxed">
            <Badge variant="accent">教师已改分</Badge>
            <span className="tabular-nums">
              专家建议 {formatScore(item.suggestedScore)} → 教师终值 {formatScore(item.score)}
            </span>
          </p>
        ) : null}

        {item.score === null ? (
          <Progress value={0} max={item.maxScore} barClassName="bg-rule" label={`${item.itemName} 得分率（无终值）`} />
        ) : (
          <Progress value={item.score} max={item.maxScore} barClassName={level.bar} label={`${item.itemName} 得分率`} />
        )}

        {item.deductionReason === null ? (
          <p className="flex items-start gap-2 rounded-md border border-primary/40 bg-primary/5 p-3 text-[13px] leading-relaxed text-foreground">
            <ShieldCheck className="mt-0.5 h-3.5 w-3.5 shrink-0" />
            {scored ? '本项满分，无扣分。' : '本项未填写扣分理由（契约要求非满分时必须说明理由）。'}
          </p>
        ) : (
          <div className="rounded-md border border-rule bg-secondary/50 p-3">
            <div className="flex items-center gap-1.5 text-xs font-medium text-foreground">
              <CircleAlert className="h-3.5 w-3.5" />
              扣分理由
            </div>
            <p className="mt-1.5 text-[13px] leading-relaxed text-foreground">{item.deductionReason}</p>
          </div>
        )}
        ...
        {adjustment === undefined ? null : (
          <div
            className={cn(
              ...
              // 改判用暖棕着色标记（"这一处被动过"），不是好坏判断
              adjustment.changed ? 'border-primary/45 bg-primary/5 text-foreground' : 'border-border bg-secondary/40',
            )}
          >
        ...
        {needsManualConfirm ? (
          // 「显著标注」用「加粗 + 实线深边 + 符号」达成，不靠红色 —— 色彩纪律见 lib/constants.ts
          <p className="flex items-start gap-2 rounded-md border border-foreground/70 bg-secondary p-3 text-[12px] font-semibold leading-relaxed text-foreground">
            <CircleAlert className="mt-0.5 h-3.5 w-3.5 shrink-0" />
            <span aria-hidden className="shrink-0">{NEEDS_REVIEW_META.symbol}</span>
            <span className="font-medium">
              复核后置信度仍低于 0.60（{formatRatio(confidenceAfterReview)}），
              须在界面显著标注并建议教师人工确认。
            </span>
          </p>
        ) : null}
