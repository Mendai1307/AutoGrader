...
import type { FingerprintCheck } from '@/lib/fingerprint';
...
export interface ProvenanceBlockProps {
  ...
  hint?: string;
  /**
   * 指纹复算结果（构建期用 lib/fingerprint.ts 现算）。
   * 传入后会在"结果指纹"一行给出「复算值 vs 声明值」的对照，
   * 让"结果是否被改动过"这件事不靠信任、靠复算。
   */
    ...
    <Card className={cn('overflow-hidden', className)}>
      <CardHeader className="border-b border-border bg-secondary/40">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {hasData ? (
            <Badge variant="accent">已固化</Badge>
          ) : (
          ...
          value={
            hasData ? (
              <span className="flex flex-col gap-1.5">
                <span className="flex flex-wrap items-center gap-2">
                  <span>{provenance.resultFingerprint}</span>
                ...
                {fingerprintCheck === undefined || fingerprintCheck === null ? null : (
                  <span className="flex flex-col gap-0.5 text-[11px] leading-relaxed text-muted-foreground">
                    <span>
                      本页按契约算法（字典序排键 + 自指字段置空 + 无空格 UTF-8 + SHA-256）现场复算：
                    </span>
                    <span className="break-all">{fingerprintCheck.computed}</span>
                    <span>{fingerprintCheck.note}</span>
                  </span>
                )}
              </span>
            ) : (
