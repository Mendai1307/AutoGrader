...
- **核对探针要选对目标**（2026-09-26 踩过）：① 测 T5「白名单外关键字必须报错」要用 `uniqueItems`
  （其自测用的也是它），**不要用 `definitions`** —— 它在白名单里，属注解关键字（与 `$defs` 同级）；
  ② 测 T1「四类输入可解析」时，纯文本无标题返回 `status=partial` **是正确行为**，**不要**要求必须 `ok`
  ——「可解析」≠「识别出标题」。③ **Prompt 里的分步链路是中文**（`S2 报告解析 → … → S6 评语生成`），
  **不要拿英文 Agent 名（Parser/Feedback）去断言**；④ 找措辞时注意**反引号**（实际是「`python3` 优先，`python` 回退」，
  按字面找「python3 优先」会漏）；⑤ **argparse 的语法层错误**（flag 拼错）行为是「打 usage 到 stderr + exit 2、
  **不产出结构化 JSON**」，**不要把它混进「语义层畸形输入」的探针**（两类要分开断言）。
  核对台：`.work/audit_plan_pre_1_6.py`（1.6 前的 32 条）、`.work/audit_plan_1_6_1_7.py`（1.6/1.7 的 16 条）
- **规划书核对进度（2026-09-26）**：1.6 之前 + 1.6 + 1.7 **合计 48 条判据，FAIL 0**
  （PASS 30 / 待平台侧实跑 16 / 判据自明前置 2）。**两处缺口**：① **M6 的「对抗样本集」未单列**
  （`assets/` 四类没有对抗样本目录；素材需**新造**，不能靠复制 v0.1 的 12 份样例）；
  ② **PDF 分支无样本未验**（本机无 PDF）
- **⚙️ 协作纪律（自己踩出来的）**：**日志里的对话编号与总纲 §九 索引行必须在同一轮内一起登记** ——
  本轮曾出现「日志编了 C-031/C-032、索引却只到 C-030」的脱节，事后补录
