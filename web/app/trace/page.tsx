...
/**
 * 置信度三档的样式**不再本地定义** —— 统一走 `CONFIDENCE_GRADE_META`。
 * 原来这里写的是 emerald / amber / rose 三色，违反"单强调色 + 灰阶、不用红绿"的设计裁决；
 * 现在改为「三格刻度符号 ●●● / ●●○ / ●○○ + 灰阶深浅」，形与色都在唯一来源里。
 */
                ...
                <Table>
                  ...
                  <TableBody>
                    {(['high', 'medium', 'low'] as const).map((grade) => (
                      <TableRow key={grade}>
                        <TableCell>
                          <Badge className={CONFIDENCE_GRADE_META[grade].chip}>
                            <span className="mr-1.5 tracking-[0.2em]">
                              {CONFIDENCE_GRADE_META[grade].symbol}
                            </span>
                            ...
                            <li key={`${step.step}-${step.agent}`} className="relative">
                              ...
                              <Card className="p-4">
                                <div className="flex flex-wrap items-center gap-2">
                                  ...
                                  <Badge className={cn('ml-auto', CONFIDENCE_GRADE_META[grade].chip)}>
                                ...
                                <div className="mt-3 flex items-center gap-2">
                                  <Progress
                                    ...
                                    max={100}
                                    className="h-1.5"
                                    barClassName={CONFIDENCE_GRADE_META[grade].bar}
                                ...
                                {step.confidence < CONFIDENCE_THRESHOLD.REVIEW ? (
                                  <p
                                    className={cn(
                                      'mt-2.5 flex items-start gap-2 rounded-md border p-2.5 text-[11px] font-medium leading-relaxed text-foreground',
                                      TONE.notice.card,
                                    )}
                                  >
                      ...
                      <div className="space-y-3">
                        <Card className="p-4">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <Badge variant={example.review.triggered ? 'warning' : 'accent'}>
                          ...
                          adjustments.map((adjustment) => (
                            <Card key={adjustment.rubricItemId} className="p-4">
                              ...
                              {adjustment.confidenceAfter < CONFIDENCE_THRESHOLD.MEDIUM ? (
                                <p
                                  className={cn(
                                    'mt-2 flex items-start gap-2 rounded-md border p-2.5 text-[11px] font-medium leading-relaxed text-foreground',
                                    TONE.bad.card,
                                  )}
                                >
                                  <TriangleAlert className={cn('mt-0.5 h-3 w-3 shrink-0', TONE.bad.icon)} />
